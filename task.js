...
class LayerEdgeConnection {
    constructor(proxy = null, privateKey = null, refCode = "9ijXgmxh") {
    ...
    async msg(msg, url, type, signKey, payload = {}) {
        try {
        const sign = await this.wallet.signMessage(msg);
        logger.info("Sign Message", sign);
        // const dataSign = { sign, timestamp, walletAddress: this.wallet.address };
        let dataSign =  payload;
        dataSign[signKey] = sign;
        const config = {
            data: dataSign,
            headers: { 'Content-Type': 'application/json' }
        };
...
async function readPrivateKeys(filename = "data.txt") {
    try {
      ...
      for (let privateKey of privateKeys) {
        try {
          ...
          wallets.push({ address: wallet.address, privateKey });
        } catch (error) {
...
async function run() {
    ...
    try {
        ...
        let wallets = await readPrivateKeys('data.txt');
        ...
        while (true) {
            for (let i = 0; i < wallets.length; i++) {
                ...
                try {
                    ...
                    // logger.progress(address, 'Performing Register', 'processing');
                    ...
                    logger.progress(address, 'Submit Proof', 'processing');
                    const formattedTimestamp = new Date().toISOString();
                    let message = `I am submitting a proof for LayerEdge at ${formattedTimestamp}`;
                    await socket.msg(message, "https://dashboard.layeredge.io/api/send-proof", "Submit Proof", "signature", { address: address, message: message, proof: "GmEdGe!" });
                    await delay(1);

                    logger.progress(address, 'Claim Light Node Task', 'processing');
                    let ts = Date.now();
                    message = `I am claiming my light node run task node points for ${address} at ${ts}`;
                    await socket.msg(message, "https://referralapi.layeredge.io/api/task/node-points", "Claim Light Node Task", "sign", { walletAddress: address, timestamp: ts });

                    logger.progress(address, 'Claim Proof Task', 'processing');
                    ts = Date.now();
                    message = `I am claiming my proof submission node points for ${address} at ${ts}`;
                    await socket.msg(message, "https://referralapi.layeredge.io/api/task/proof-submission", "Claim Proof Task", "sign", { walletAddress: address, timestamp: ts });

                    // logger.progress(address, 'Claim NFT2 Task', 'processing');
                    // ts = Date.now();
                    // message = `I am claiming my SBT verification points for ${address} at ${ts}`;
                    // await socket.msg(message, "https://referralapi.layeredge.io/api/task/nft-verification/2", "Claim NFT2", "sign", { walletAddress: address, timestamp: ts });

                    // logger.progress(address, 'Claim NFT1 Task', 'processing');
                    // ts = Date.now();
                    // message = `I am claiming my SBT verification points for ${address} at ${ts}`;
                    // await socket.msg(message, "https://referralapi.layeredge.io/api/task/nft-verification/2", "Claim NFT1", "sign", { walletAddress: address, timestamp: ts });
